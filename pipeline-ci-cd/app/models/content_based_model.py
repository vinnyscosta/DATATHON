...
from utils.AWS import S3Client, DynamoDBClient
...
class BasedContentRecomendation:
    def __init__(
        self,
        bucket_name: str = "datathon-base-458807800524",
    ...
    def load_model(self, file_name="content_based_model.pkl") -> None:
        ...
        if not os.path.exists(file_path):
            logging.error(f"Arquivo {file_name} não encontrado após download!")
            return None

        logging.info(f"Arquivo {file_name} encontrado após download!")
        ...
        logging.info(f"Tamanho do arquivo: {os.path.getsize(file_path)} bytes")

        # Tenta abrir o arquivo sem carregar
        try:
            with open(file_path, 'rb') as f:
                first_bytes = f.read(100)
                logging.info(f"Primeiros 100 bytes do arquivo: {first_bytes}")
        except Exception as e:
            logging.error(f"Erro ao abrir o arquivo: {e}")
            return None

        try:
            logging.info("Inicio Carregamento Modelo na memória!")
            with open(file_path, 'rb') as f:
                logging.info("Continuando Inicio Carregamento Modelo carregado na memória!")
                data = pickle.load(f)
